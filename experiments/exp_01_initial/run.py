"""
Coverage check for the recalibration (migration ON, n_alive as a target).

Draws N=50 LHS samples from `priors.calib_pars`, runs one seed per draw with
`use_migration=True`, and extracts annual time series of the columns that
appear in `data/zimbabwe_hiv_calib.csv`:

    hiv.prevalence, hiv.n_infected, hiv.new_infections, hiv.new_deaths,
    hiv.prevalence_15_49, n_alive

Saves the ensemble to `outputs/coverage_check.parquet` and a 3x2 figure at
`figures/coverage_check.png` overlaying UNAIDS + implied-pop targets against
the ensemble envelope.

The question this answers: with migration on, does the prior even reach the
data for n_alive (previously overshot by ~20%) — before we spend compute on
a full calibration?
