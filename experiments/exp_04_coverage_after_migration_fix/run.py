...
def load_targets():
    """Assemble targets: UNAIDS calib CSV + n_art.csv + derived ART coverage."""
    d = pd.read_csv(REPO / 'data' / 'zimbabwe_hiv_calib.csv')
    art = pd.read_csv(REPO / 'data' / 'n_art.csv')
    d = d.merge(art[['year', 'n_art']].rename(columns={'year': 'time'}),
                on='time', how='left')
    # Derived UNAIDS ART coverage while both n_art and PLHIV are available
    d['unaids_art_coverage'] = np.where(
        (d.n_art > 0) & d['hiv.n_infected'].notna(),
        d.n_art / d['hiv.n_infected'], np.nan)
    return d
