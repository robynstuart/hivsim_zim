...
| Transmission from diagnosed-not-on-ART (%) | Yes | 2024 aggregate 1.4 | <5.0 (all models) | Now well within the paper's range. Fixed by the upstream stisim bug (`post_art` bool was never being set to True in `stop_art`, so recently-interrupted agents were folded into "diagnosed-not-on-ART" instead of "post-ART"). |
| Transmission from on-ART (%) | Yes | 2024 aggregate 58.8 | 19.5-54.2 | Just above the paper's upper bound; consequence of p_art=0.92 pushing more PLHIV onto ART. |
| Transmission from post-ART / ART-interrupted (%) | Yes | 2024 aggregate 27.8 | 4.7-21.5 | Slightly above the paper's upper bound. Trajectory: peaks at 59% in 2020, drops to 28% by 2024 when p_art=0.92 kicks in (more agents re-enrolled on ART, fewer in interruption). stisim's `dur_on_art` mean is 3 years (lognormal) — an agent enrolled in 2010 has ~50% chance of being off ART by 2013. |
